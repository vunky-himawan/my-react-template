...
>() {
  ...
  const debouncedSearch = useDebounce((value: string) => {
    ...
    setParams({ ...filters, search: value, page: 1, limit: 10 } as Partial<TFilters>);
  }, 500);

  const handleChange = {
    onFilterChange: (newFilters: Partial<TFilters>) => {
      updateFilters(newFilters);
      setParams({ ...filters, ...newFilters, page: 1, limit: 10 });
    },
    onSortingChange: (sort: string, order: "asc" | "desc") => {
      updateFilters({ sort, order } as Partial<TFilters>);
      setParams({ ...filters, sort, order, page: 1, limit: 10 } as Partial<TFilters>);
    },
    onPaginationChange: (page: number, limit: number) => {
      updateFilters({ page, limit } as Partial<TFilters>);
      setParams({ ...filters, page, limit } as Partial<TFilters>);
    },
